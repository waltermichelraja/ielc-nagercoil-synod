...
export const NAV_ITEMS=[
  ...
  {to:"/leadership",label:"Synod Bearers"},
  ...
  return(
    <header className="site-header">
      <div className="container">
        ...
        <button className="nav-toggle" id="nav-toggle" aria-expanded={open} aria-controls="main-nav" onClick={()=>setOpen(v=>!v)}>Menu</button>
            ...
            {NAV_ITEMS.map(item=>(
              <li key={item.to}>
                <NavLink
                  ...
                  aria-current={(item.end?pathname===item.to:pathname.startsWith(item.to))?"page":undefined}
                >
                  {item.label}
                </NavLink>
              </li>
      ...
      {site.notices&&site.notices.length>0&&(
        <div className="ticker" role="marquee" aria-label="Notice board">
          <div className="ticker-track">
            {site.notices.map((notice,i)=><span key={`a-${i}`}>{notice}</span>)}
            {site.notices.map((notice,i)=><span key={`b-${i}`}>{notice}</span>)}
          </div>
        </div>
      )}
    </header>
